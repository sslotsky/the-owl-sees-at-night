...
import DragTracker from "~/components/drag-tracker/drag-tracker";
  ...
  useVisibleTask$(async ({ track }) => {
    ...
    if (props.windowRef.value) {
      ...
      img
        .decode()
        .then(() => {
          const maxX = divide(img.clientWidth, props.store.printSizeX);
          const maxY = divide(img.clientHeight, props.store.printSizeY);

          const scale = Math.min(maxX, maxY);
          props.store.cropperWidth = divide(props.store.printSizeX * scale, props.store.zoomFactor);
          props.store.cropperHeight = divide(props.store.printSizeY * scale, props.store.zoomFactor);
