...
import type { VariantArray, Variant } from "./types";
import { useAddToOrder } from "~/components/shop-context/context";
        ...
        <div class="tab-content">
          {selectedGroup &&
            selectedGroup.map((variant) => (
              <div key={variant.id} class="flex-wrapper">
                <div
...
export default component$(
  (props: { files: MasonryPhoto[]; productData: CustomPrintQuery }) => {
    ...
    useTask$(({ track }) => {
      ...
      if (image.value) {
        const maxZoomFactor =
          divide(image.value.naturalWidth, image.value.clientWidth);
        store.zoomFactor =
    ...
    return (
      <div class="custom-prints">
        <div>
          <div class="scroll-container max-height">
            {store.gridView ? (
              <div class="grid-wrap">
                <div class="photo-grid">
                  {props.files.map((f) => (
                    <div class="image-wrapper" key={f.fileId}>
                      <img
                        onClick$={() => {
                          store.file = f;
